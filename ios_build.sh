...
if [ "$1" == "build" ]; then
xcodebuild -project build-ios/NeuralPi.xcodeproj \
  -scheme NeuralPi_Standalone archive -configuration Release \
  -sdk iphoneos -jobs 12 -archivePath NeuralPi.xcarchive | xcpretty
fi
fi
...
if [ "$1" == "version" ]; then
  ...
  PLIST=NeuralPi.xcarchive/Info.plist
  ...
  mkdir -p "$archive_dir"
  mv NeuralPi.xcarchive "$archive_dir/NeuralPi-$COMMIT.xcarchive"
fi
